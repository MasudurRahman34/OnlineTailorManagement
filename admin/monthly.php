...
`
                <div class="block">        
					...
					<tbody>
						...
						echo "<span style='color:green; font-size:15px'>Todays date </span>".$cur_date."<br>";
						  ...
						  $query="SELECT title AS 'product Name', customer_name, order_date, price FROM fabric_order, customer, product WHERE fabric_order.customer_email=customer.customer_email and product.product_id=fabric_order.product_id and status=1 order by foid desc";
						...
						$sum= "SELECT sum(price) AS dtotal FROM shirt_design, product WHERE shirt_design.product_id=product.product_id and status and shirt_design.ddate LIKE '$cur_date%' and status=1";
						$sum=$db->select($sum);
						$result = mysqli_fetch_array($sum);
                ...
                <div class="block">        
                   <table class="data display datatable" id="example">
					...
					<tbody>
						<?php 
						$query="SELECT design_id, shirt_design.product_id, price, shirt_design.customer_email, measurment_id FROM shirt_design, shirt_measurment, product WHERE shirt_design.customer_email=shirt_measurment.customer_email and shirt_design.product_id=product.product_id and status=1  and shirt_design.ddate LIKE '$cur_date%' order by design_id desc";
